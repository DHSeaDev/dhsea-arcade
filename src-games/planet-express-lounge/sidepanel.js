...
document.querySelectorAll(".tab").forEach(tab => {
  tab.addEventListener("click", () => {
    ...
    if (id === "lab")      { window.LabModule?.startLabWidgets(); window.LabModule?.renderPatentOffice(); }
    if (id === "cold")     loadPins();
...
function _isDemoMode() { return _demoActive; }
  ...
  for (const pin of pins) {
    ...
    row.innerHTML = `
      ...
      <div class="cold-pin-actions">
        <button class="cold-pin-share" data-id="${pin.id}" title="Copy shareable text for social media">🔗 SHARE</button>
...
// Copy a Cold Storage item as shareable text — same pattern as the per-message
// 🔗 share button, applied to a saved pin (transcript, invention, or quote).
...
async function loadSettings() {
  const saved = await chrome.storage.local.get([
    ...
    "lightMode","chaosMode","audioMuted","chromeVoiceOverride",
  ]);
  ...
  // Chrome Voice Override
  const _cvo = saved.chromeVoiceOverride === true;
  setChromeVoiceOverride(_cvo);
  const _cvoEl = $("chromeVoiceOverride");
  if (_cvoEl) _cvoEl.checked = _cvo;
  // Defer status update until voices have loaded
  setTimeout(_updateVoiceEngineStatus, 500);

  // ElevenLabs (BYOK premium, optional) — activate immediately so voice
  // priority (ElevenLabs → OS → Chrome) applies from the first utterance.
  // Voice ID fields are left empty unless the user customized them — empty
  // means "use the shipped default" (handled by setElevenLabsVoiceIds).
  const elevenLabsKeyEl = $("elevenLabsKey");
  if (elevenLabsKeyEl) elevenLabsKeyEl.value = saved.elevenLabsKey || "";
  setElevenLabsKey(saved.elevenLabsKey || "");
  _updateElevenLabsKeyStatus(!!saved.elevenLabsKey);
...
// Chrome Voice Override toggle
const chromeVoiceOverrideEl = $("chromeVoiceOverride");
if (chromeVoiceOverrideEl) {
  chromeVoiceOverrideEl.addEventListener("change", () => {
    const enabled = chromeVoiceOverrideEl.checked;
    setChromeVoiceOverride(enabled);
    chrome.storage.local.set({ chromeVoiceOverride: enabled }).catch(() => {});
...
if (elevenLabsKeyEl) {
  elevenLabsKeyEl.addEventListener("blur", () => {
    ...
    _updateElevenLabsKeyStatus(!!key);
    _updateVoiceEngineStatus();
  });
}
 ...
 */
function _updateElevenLabsKeyStatus(hasKey) {
  const el = $("elevenLabsKeyStatus");
  if (!el) return;
  el.innerHTML = hasKey
    ? `<span style="color:var(--gold)">●</span> ElevenLabs active — default voices assigned, customize per category below if you like.`
    : `<span style="color:var(--fg3)">○</span> No key set — using OS/Chrome voices.`;
}
...
function _updateVoiceEngineStatus() {
  ...
  chrome.tts.getVoices(voices => {
    ...
    const os = /microsoft/i.test(names) ? "Windows"
             ...
             : "ChromeOS/Other";
    const override = chromeVoiceOverrideEl?.checked;
    const elKeySet  = !!($("elevenLabsKey")?.value || "").trim();
    const active   = elKeySet   ? "ElevenLabs (your voices)"
                   : override   ? "Chrome built-in voices (override)"
                   : `OS voices (${os})`;
    statusEl.innerHTML =
...
// ── Themed lab widgets (rebuilt) ────────────────────────────────────────────
// Pattern: lazy getElementById inside every render call, try/catch everywhere,
// no closure-captured element refs, no inline <style> blocks.

// ────────────────────────────────────────────────────────────────────────────
// v2 NEWS DESK — Morbo & Linda
// ────────────────────────────────────────────────────────────────────────────
