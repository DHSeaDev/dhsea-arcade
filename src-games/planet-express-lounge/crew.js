...
import {
  ...
  INVENTION_CRITICS, CRITIQUE_SYS,
  checkEasterEgg,
  getWeeklyMission,
} from "./prompts.js";
...
export class Crew {
   ...
   */
  async genInventionCritique(inventionText, emit) {
    if (!inventionText) return;

    // Pick a random enabled critic — never the Professor
    const available = INVENTION_CRITICS.filter(a => this.enabled[a] !== false);
    if (!available.length) return;
    const critic = available[Math.floor(Math.random() * available.length)];

    const sys  = CRITIQUE_SYS[critic];
    if (!sys) return;

    const prompt = `The Professor just announced: "${inventionText.slice(0, 120)}"\nReact in one sentence.`;

    emit({ type: "speaker", agent: critic });
    try {
      const text = await this.llm.complete(sys, prompt, 45);
      if (text && text.trim()) {
        // Emit word by word so the same typing animation fires
        const words = text.trim().split(" ");
        for (let i = 0; i < words.length; i++) {
          emit({ type: "token", agent: critic, text: (i === 0 ? "" : " ") + words[i] });
        }
        emit({ type: "turn_end", agent: critic });
        return text.trim();
      }
    } catch {}
    emit({ type: "turn_end", agent: critic });
